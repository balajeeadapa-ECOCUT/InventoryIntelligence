...
class DailyStockAlertService {
  ...
  private initializeTransporter() {
    ...
    if (smtpHost && smtpUser && smtpPass) {
      ...
      console.log('[DailyStockAlerts] Email transporter initialized');
    } else {
      console.log('[DailyStockAlerts] SMTP not configured - emails will be simulated');
    }
  }
...
<head>
  <meta charset="UTF-8">
  <title>EcoCut Smart Inventory - Daily Stock Alert</title>
</head>
  ...
  <div style="max-width: 800px; margin: 0 auto; background-color: white; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
    ...
    <div style="padding: 20px;">
      ...
      <table style="width: 100%; border-collapse: collapse;">
        ...
        <tbody>
          ${generateTableRows(lowStockItems)}
        </tbody>
      </table>
    </div>
    ` : ''}
    ...
    const generateTextRows = (items: any[]) => items.map(item => 
      `  ${item.sku?.padEnd(15) || 'N/A'.padEnd(15)} | ${item.name.substring(0, 25).padEnd(25)} | ${String(item.currentStock).padStart(5)} | ${formatCurrency(parseFloat(item.unitPrice) || 0).padStart(12)} | ${formatCurrency(calculateTotalValue(item)).padStart(12)}`
    ).join('\n');
...
${lowStockItems.length === 0 && outOfStockItems.length === 0 ? `
✅ ALL STOCK LEVELS ARE HEALTHY - No items require immediate attention.
` : ''}
